...
def test_final_layer_has_one_index() -> None:
    assert PRESETS["final_layer"].layer_indices == [35]


def test_overlay_preserves_base() -> None:
    base = {"model": {"name_or_path": "Qwen/Qwen3-4B-Instruct-2507"}, "training": {"max_steps": 1000}}
    out = overlay_eagle3(base, PRESETS["tri_layer"])
    assert out["model"]["name_or_path"] == "Qwen/Qwen3-4B-Instruct-2507"
    assert out["training"]["max_steps"] == 1000
    assert out["eagle3"]["layer_indices"] == [7, 18, 29]
...
def test_write_variant_config(tmp_path: Path) -> None:
    ...
    assert loaded["eagle3"]["layer_indices"] == [35]
